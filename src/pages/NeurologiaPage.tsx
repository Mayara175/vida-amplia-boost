    ...
    <Layout>
      ...
      <section className="relative pt-24 md:pt-28 pb-12 md:pb-20 overflow-hidden">
        ...
        <div className="container mx-auto relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
            ...
            >
              <span className="text-accent font-medium text-xs md:text-sm tracking-widest uppercase">
              ...
              <p className="text-muted-foreground text-base md:text-lg mt-4 md:mt-6 leading-relaxed max-w-lg">
            ...
            >
              <div className="relative w-full max-w-xs sm:max-w-sm md:max-w-md">
                ...
                <div className="absolute -bottom-3 -left-3 md:-bottom-4 md:-left-4 bg-card rounded-xl md:rounded-2xl p-3 md:p-4 shadow-xl border border-border hidden sm:block">
                  <p className="text-xs md:text-sm text-muted-foreground">Especialidade</p>
                  <p className="text-base md:text-lg font-display font-bold text-foreground">Neurologia</p>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <section className="section-padding bg-secondary">
        <div className="container mx-auto max-w-4xl">
          ...
          <motion.div {...fadeUp} transition={{ duration: 0.6 }} className="bg-card rounded-2xl md:rounded-3xl p-6 md:p-8 lg:p-10 border border-border">
              ...
              {credentials.map((c) => (
                <div key={c.text} className="flex items-start gap-3 md:gap-4">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-accent/10 flex items-center justify-center flex-shrink-0">
                    <c.icon className="w-4 h-4 md:w-5 md:h-5 text-accent" />
                  </div>
                  <span className="text-foreground pt-1.5 md:pt-2 text-xs md:text-sm">{c.text}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8">
            {specialties.map((s, i) => (
              <motion.div
                ...
                className="bg-card rounded-xl md:rounded-2xl p-5 md:p-8 border border-border hover:border-accent/30 hover:shadow-lg transition-all duration-500"
              >
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-accent/10 flex items-center justify-center mb-3 md:mb-4">
                  <s.icon className="w-5 h-5 md:w-6 md:h-6 text-accent" />
                </div>
                <h3 className="text-lg md:text-xl font-display font-bold text-foreground mb-2 md:mb-3">{s.title}</h3>
                <p className="text-muted-foreground text-xs md:text-sm leading-relaxed">{s.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
